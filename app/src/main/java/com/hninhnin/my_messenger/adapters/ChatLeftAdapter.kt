...
class ChatLeftAdapter(val message: String, val user: User) : Item<GroupieViewHolder>() {
    override fun bind(viewHolder: GroupieViewHolder, position: Int) {
        viewHolder.itemView.findViewById<TextView>(R.id.txtMsgLeft).text = message
        Picasso.get().load(user.profileImage).into(viewHolder.itemView.findViewById<CircleImageView>(R.id.imgMsgLeft))
    }
